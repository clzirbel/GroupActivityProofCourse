...
\example{For each of the logical statements below, write its truth value T or F.
...
%===============================================================
\pagebreak
\exercise{Give the truth value T or F of each of the new statements below, using the statements from the previous page.
...
\exercise{Give the truth value T or F of each of the new statements below.
...
State $\lnot Q$ and $\lnot P$ and show $\lnot Q \to \lnot P$.}{0.7in}

\exercise{Let $n$ be an integer.  Consider $S: n^2$ is a multiple of 3 and $T: n$ is a multiple of 3.
State $\lnot T$ and $\lnot S$.  By the Division Algorithm, there are two ways that $\lnot T$ can happen.  Write down each one, and prove that in either case, they imply $\lnot S$.}{1.0in}
...
\exercise{Suppose that $a$ is an irrational number.  Let $b$ be a rational number not equal to 0.  Show that $ba$ is irrational.}{0.0in}
...
Prove that $P \wedge Q$ is false by first pretending that it is true.
One idea is that $n$ would have to be larger than 2 but smaller than 3.}{0.5in}
...
Thus, $p$ is \blank{1in} by result \blank{1in} and so can be written as $p = \blank{1in}$ for some \blank{1in}.
...
\definition{Prime}{An integer $n>1$ is {\em prime} if it is not composiite, that is, its only non-negative integer factors are 1 and itself.}
...
Rewrite $\lnot B$ in terms of the cases from \ref{nsquaredmultipleof5}, and then argue that each case iimplies $\lnot A$.  You may use the cases from \ref{nsquaredmultipleof5} without rewriting them.
}{0.9in}
