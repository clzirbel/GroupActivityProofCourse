...
\activitytitle{Quiz on a new operation with 3--dimensional vectors}{15 points}
...
\exampleNN{For example, $\vect{1,3,5} * \vect{2,7,10} = \vect{1 \cdot 10, 3 \cdot 7, 5 \cdot 2} = \vect{10,21,10}$.}{0in}

\showNN{Show that the twist product is distributive over vector addition.
That is, show that $(\vec{a} \oplus \vec{b}) * \vec{c} = \vec{a} * \vec{c} \oplus \vec{b} * \vec{c}.$  Start with ``Let \ldots,'' take one step at a time, write the justification for the step, and make a general conclusion.}{3.4in}
