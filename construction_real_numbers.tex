...
\definition{Closed below}{A set $A$ of rational numbers is said to be {\em closed below} if for all $q \in A$, for all $p \in \Q$ with $p < q$, we have $p \in A$.  In words, if $A$ contains the rational number $q$, then it contains every rational number less than $q$ as well.}
